// That test case verify  dashboard List

describe('Verify dashboard List:', function () {
    ...
    it('Verify dashboard List', function () {
        ...
        element.all(by.css('[ng-click="triggerSyncAlertsModal()"]')).first().click();

        var popover = protractor.ExpectedConditions;
        var content = element(by.css('.popover-inner'));
        browser.wait(popover.visibilityOf(content), 50000);
        expect(content.getText()).toContain('All applications are synced in real-time');

        element(by.id('sync')).click();
        element.all(by.css('[ng-click="triggerSyncAlertsModal()"]')).first().click();

        var popover = protractor.ExpectedConditions;
        var content = element(by.css('.popover-content'));
        browser.wait(popover.visibilityOf(content), 50000);
        expect(content.getText()).toContain('All applications are synced in real-time');

        var newdashboard = protractor.ExpectedConditions;
        var newdashboardLinkTextlist = element(by.css('.dashboard-select.hidden-print'));
        browser.wait(newdashboard.presenceOf(newdashboardLinkTextlist), 180000);
        expect(newdashboardLinkTextlist.isPresent()).toBeTruthy();
